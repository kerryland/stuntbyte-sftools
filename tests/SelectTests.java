...
public class SelectTests extends TestCase {
    ...
    public SelectTests(String arg0) {
        super(arg0);
    }

    private class MyExpressionVisitor implements ExpressionVisitor {
        ...
        public void visit(JdbcParameter jdbcParameter) {
            System.out.println("jdbc param " + jdbcParameter.toString());
        }

        public void visit(DoubleValue doubleValue) {
            System.out.println("dv " + doubleValue.toString());
        }

        public void visit(LongValue longValue) {
            System.out.println("lv " + longValue.toString());
        }

        public void visit(DateValue dateValue) {
            System.out.println("dv " + dateValue.toString());
        ...
        public void visit(InExpression inExpression) {
            System.out.println("in " + inExpression.toString());
        ...
        public void visit(Column column) {
            System.out.println("COL=" + column.getColumnName() + " of " + column.getTable());
        }
        ...
        public void visit(CaseExpression caseExpression) {
            System.out.println("case " + caseExpression.toString());

        }

        public void visit(WhenClause whenClause) {
            System.out.println("when " + whenClause.toString());
        ...
        public void visit(AllComparisonExpression allComparisonExpression) {
            System.out.println("allcomp " + allComparisonExpression.toString());
        }

        public void visit(AnyComparisonExpression anyComparisonExpression) {
            System.out.println("any comp " + anyComparisonExpression.toString());
        }
    ...
    private class MySelectVisitor implements SelectVisitor {

        public void visit(PlainSelect plainSelect) {

            plainSelect.getFromItem().accept(new FromItemVisitor() {
                public void visit(Table table) {
                    System.out.println("Table is " + table.getWholeTableName());
                }
                ...
                public void visit(SubJoin subJoin) {
                    System.out.println("Subjoin? " + subJoin.getJoin().isLeft());
            ...
            for (Object item : items) {
                if (item instanceof SelectExpressionItem) {
                    ...
                    si.getExpression().accept(new MyExpressionVisitor());
                }
                if (item instanceof AllTableColumns) {
                    AllTableColumns all = (AllTableColumns) item;
                    all.accept(new SelectItemVisitor() {
                        ...
                        public void visit(SelectExpressionItem selectExpressionItem) {
                            System.out.println("SelectExpressionItem: " + selectExpressionItem.toString());
                        }
                    });
                }
                System.out.println("SITEM: " + item.getClass().getName());
            }

            items = plainSelect.getJoins();
            for (Object item : items) {
                System.out.println("jITEM: " + item.getClass().getName());
                Join ji = (Join) item;
                List usings = ji.getUsingColumns();
                if (usings != null) {
                    for (Object using : usings) {
                        System.out.println("Using " + using);
                    }
                } else {
                    System.out.println("Usings is null");
                }

                if (ji.getOnExpression() != null) {
                    ji.getOnExpression().accept(new MyExpressionVisitor());
                }
                ji.getRightItem().accept(new FromItemVisitor() {
                    public void visit(Table table) {
                        System.out.println("join to " + table.getWholeTableName());
                    }
    ...
    public void testKerryPlay() throws JSQLParserException {
        String statement = "SELECT * FROM tab1 INNER JOIN tab2 USING (id, id2)";
//        String statement =
//                "SELECT a.Id, UpperCase(a.Name), a.*, b.* " +
//                        "FROM aaa__c a " +
//                        "JOIN bbb__c  b on bbb__c.Id = aaa__c.bbb__c " +
//                        " WHERE bbb__c.name = 'Wibble'";
        Object obj = parserManager.parse(new StringReader(statement));
        if (obj instanceof Select) {
            Select select = (Select) obj;

            SelectBody sb = select.getSelectBody();
            sb.accept(new MySelectVisitor());
        }
    }
